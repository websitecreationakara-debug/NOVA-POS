-- Printable invoices: give every order a sequential, human-readable
-- invoice number (assigned at charge time, not derived from the UUID).
...
-- Backfill invoice numbers for any pre-existing orders (none in production
-- yet, but keeps the migration correct if that ever changes).
update orders set invoice_number = 'INV-' || lpad(nextval('invoice_number_seq')::text, 6, '0')
