...
import { deleteWasteLogAction } from "@/app/(app)/accountance/actions";

// Same look/behavior as DeleteExpenseDialog/DeleteWebsiteAddonDialog --
// mount only while confirming: `{confirming && <DeleteWasteLogDialog … />}`.
export default function DeleteWasteLogDialog({
  entryId,
  productName,
  quantity,
  onClose,
  ...
  function handleDelete() {
    startTransition(async () => {
      try {
        await deleteWasteLogAction(entryId);
        router.refresh();
        onClose();
      } catch (e) {
  ...
  return (
    ...
    >
      <div
        role="alertdialog"
        aria-modal="true"
        aria-labelledby="delete-waste-title"
        aria-describedby="delete-waste-description"
      ...
      >
        ...
        <h2 id="delete-waste-title" className="mt-4 text-base font-semibold text-foreground">
          Delete this waste entry?
        </h2>
        <p id="delete-waste-description" className="mt-1.5 text-sm text-muted-foreground">
          {quantity} unit{quantity === 1 ? "" : "s"} of &quot;{productName}&quot; will be added back to
          stock and this entry removed. This can&apos;t be undone.
        </p>
